...
parser.add_argument('--weight', default='uniform', type=str, help='multi-task weighting: uniform, gradnorm, mgda')
...
def save_checkpoint(state, is_best, checkpoint=opt.out, filename='checkpoint.pth.tar'):
    filepath = os.path.join(checkpoint, 'segnet_weight_{}_'.format(opt.weight) + filename)
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'segnet_weight_{}_'.format(opt.weight) + 'model_best.pth.tar'))

if not os.path.isdir(opt.out):
    mkdir_p(opt.out)
title = 'NYUv2'
logger = Logger(os.path.join(opt.out, 'segnet_weight_{}_log.txt'.format(opt.weight)), title=title)
...
nyuv2_train_loader = torch.utils.data.DataLoader(
    ...
    shuffle=True, num_workers=30, drop_last=True)
    ...
    for k in range(train_batch):
        ...
        if opt.weight == 'mgda':
            ...
            for i, t in enumerate(tasks):
                gygw.append(torch.autograd.grad(train_loss[i], W, retain_graph=True)[0].detach())
