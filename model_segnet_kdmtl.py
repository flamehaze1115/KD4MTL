...
logger.set_names(
    ['Epoch', 'T.Ls', 'T. mIoU', 'T. Pix', 'T.Ld', 'T.abs', 'T.rel', 'T.Ln', 'T.Mean', 'T.Med', 'T.11', 'T.22', 'T.30',
     'V.Ls', 'V. mIoU', 'V. Pix', 'V.Ld', 'V.abs', 'V.rel', 'V.Ln', 'V.Mean', 'V.Med', 'V.11', 'V.22', 'V.30', 'ds',
     'dd', 'dh'])
...
for epoch in range(total_epoch):
    ...
    for k in range(train_batch):
        ...
        for i, t in enumerate(tasks):
            ...
            feat_ti0 = feat_ti0 / (feat_ti0.pow(2).sum(1) + 1e-6).sqrt().view(feat_ti0.size(0), 1, feat_ti0.size(2),
                                                                              feat_ti0.size(3))
            feat_ti1 = feat_ti[1].detach()
            feat_ti1 = feat_ti1 / (feat_ti1.pow(2).sum(1) + 1e-6).sqrt().view(feat_ti1.size(0), 1, feat_ti1.size(2),
                                                                              feat_ti1.size(3))
            feat_si = transformers[i](feat_s)
            feat_si0 = feat_si[0] / (feat_si[0].pow(2).sum(1) + 1e-6).sqrt().view(feat_si[0].size(0), 1,
                                                                                  feat_si[0].size(2),
                                                                                  feat_si[0].size(3))
            feat_si1 = feat_si[1] / (feat_si[1].pow(2).sum(1) + 1e-6).sqrt().view(feat_si[1].size(0), 1,
                                                                                  feat_si[1].size(2),
                                                                                  feat_si[1].size(3))
...
print('Epoch: {:04d} | TRAIN: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} '
      'TEST: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} '
      .format(print_index, avg_cost[print_index, 0], avg_cost[print_index, 1], avg_cost[print_index, 2],
              avg_cost[print_index, 3],
              avg_cost[print_index, 4], avg_cost[print_index, 5], avg_cost[print_index, 6], avg_cost[print_index, 7],
              avg_cost[print_index, 8], avg_cost[print_index, 9],
              avg_cost[print_index, 10], avg_cost[print_index, 11], avg_cost[print_index, 12],
              avg_cost[print_index, 13],
              avg_cost[print_index, 14], avg_cost[print_index, 15], avg_cost[print_index, 16],
              avg_cost[print_index, 17], avg_cost[print_index, 18],
              avg_cost[print_index, 19], avg_cost[print_index, 20], avg_cost[print_index, 21],
              avg_cost[print_index, 22], avg_cost[print_index, 23]))
